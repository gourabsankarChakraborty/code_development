...
public class H2HApplicationDao {
	...
	public CustomerDetails getApplicationDetails(Integer applicationId) {
		...
		try {
			if (null != applicationId) {
				...
				// Query query =
				// entityManager.createQuery(sbHQLSelect.toString() );
	...
	@Async
	public CompletableFuture saveApplicationDetailsMultiThread(CustomerDetails customerDetails) {
		CustomerDetails custDetailsModel = new CustomerDetails();
		...
		try {
		 ...
		 return CompletableFuture.completedFuture(custDetailsModel);
//		return custDetailsModel;
	}
}
